...
public class BooruTests
{
    ...
    {
        Pixiv booru = (Pixiv)await Boorus.GetAsync(typeof(Pixiv));

        Search.Post.SearchResult[] result = await booru.GetFavoritesAsync(56850985);

        Assert.NotEmpty(result);

        foreach (Search.Post.SearchResult r in result)
        {
    ...
    public async Task GetPixivBookmarksInvalidIdAsync()
    {
        ...
        Search.Post.SearchResult[] result = await booru.GetFavoritesAsync(111);
    ...
    {
        ABooru booru = await Boorus.GetAsync<E621>();
        General.Authorize(booru);

        Skip.If(booru is Xbooru, "Xbooru allows adding a post with invalid ID.");

        Search.Post.SearchResult res = await booru.GetRandomPostAsync("cub", "pussy");
    ...
    public async Task GetByMd5Async(Type t)
    {
        ...
        else
        {
            ...
            Search.Post.SearchResult result2 = await booru.GetPostByMd5Async(result1.MD5);
    ...
    public async Task GetLastPostsAsync(Type t)
    {
        ...
        {
            Search.Post.SearchResult[] results = await booru.GetLastPostsAsync();
    ...
    public async Task GetLastPostsWithLimitAsync(Type t)
    {
        ...
        {
            Search.Post.SearchResult[] results = await booru.GetLastPostsAsync(100);
    ...
    public async Task GetLastPostsWithTagsAsync(Type t, string tag = "hibiki_(kantai_collection)", string tag2 = "swimsuit")
    {
        ...
        Assert.NotInRange(results.Length, 0, 1);
        Assert.NotEqual(results[0].ID, results[1].ID);
        foreach (Search.Post.SearchResult elem in results)
        {
    ...
    public async Task GetPostCountAsync(Type t, string tag = "hibiki_(kantai_collection)", string tag2 = "swimsuit")
    {
        ...
        else
        {
            ...
            int countOne = await booru.GetPostCountAsync(tag);
            int countTwo = await booru.GetPostCountAsync(tag, tag2);
    ...
    public async Task GetRandomsTooManyAsync(Type t, string tag = "school_swimsuit")
    {
        ...
        {
            Search.Post.SearchResult[] result = await booru.GetRandomPostsAsync(int.MaxValue, tag);
            Assert.NotEmpty(result);
            foreach (Search.Post.SearchResult r in result)
    ...
    public async Task GetRandom2TagsAsync(Type t, string tag = "hibiki_(kantai_collection)", string tag2 = "school_swimsuit")
    {
        ...
        {
            Search.Post.SearchResult result = await booru.GetRandomPostAsync(tag, tag2);
    ...
    public async Task GetRandoms2TagsAsync(Type t, string tag = "hibiki_(kantai_collection)", string tag2 = "school_swimsuit")
    {
        ...
        {
            Search.Post.SearchResult[] result = await booru.GetRandomPostsAsync(_randomPostCount, tag, tag2);
            Assert.NotEmpty(result);
            foreach (Search.Post.SearchResult r in result)
            {
                Assert.Contains(r.Tags, x => x.Contains(tag));
    ...
    public async Task CheckIsSafeAsync(Type t, string explicitTag = "pussy")
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            Search.Post.SearchResult image = await b.GetRandomPostAsync(explicitTag);
